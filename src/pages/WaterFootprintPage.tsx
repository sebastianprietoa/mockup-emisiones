import { ArrowUpRight, Droplets, Gauge, Target, TrendingDown, Waves } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <KpiCard
          label="Huella hídrica total"
          value={`${formatNumber(view.totalFootprint)} m³ eq`}
          helper="Inventario consolidado por organización."
          icon={<Droplets size={18} />}
        />
        <KpiCard
          label="Mayor componente"
          value={view.kpis.dominantComponent || "Sin datos"}
          helper="Componente dominante en la huella."
          icon={<ArrowUpRight size={18} />}
        />
        <KpiCard
          label="Variación interanual"
          value={formatSignedPercent(view.kpis.yearVariation)}
          helper="Reducción simulada respecto del año previo."
          trend={view.kpis.yearVariation <= 0 ? "baja" : "alza"}
          icon={<TrendingDown size={18} />}
        />
        <KpiCard
          label="Intensidad hídrica"
          value={`${formatNumber(view.kpis.intensity)} m³/M$`}
          helper="Indicador de eficiencia hídrica."
          icon={<Gauge size={18} />}
        />
        <KpiCard
          label="Avance de metas"
          value={formatPercent(view.kpis.goalProgress)}
          helper="Progreso visual frente a la meta interna."
          status={view.kpis.goalProgress >= 50 ? "bueno" : "medio"}
          icon={<Target size={18} />}
        />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-soft backdrop-blur">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-300">Mensaje ejecutivo</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-2 text-sm text-slate-200">WFN / ISO 14046 como marco conceptual.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Riesgo</p>
              <p className="mt-2 text-sm text-slate-200">Cuenca y estrés hídrico como capa de decisión.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <ChartCard title="Evolución anual" description="Tendencia consolidada por año.">
          <LineChart
            data={view.yearData.map((row) => ({ year: row.year, total: row.total }))}
            xKey="year"
            series={[{ key: "total", name: "m³ eq" }]}
          />
        </ChartCard>

        <ChartCard title="Consumo por instalación" description="Trazabilidad de consumo por centro operativo.">
          <BarChart
            data={view.installationData.map((item) => ({ installation: item.name, volume: item.value }))}
            xKey="installation"
            series={[{ key: "volume", name: "m³ eq" }]}
            layout="vertical"
          />
        </ChartCard>
        ...
        <ChartCard title="Cuenca con mayor presión" description="Distribución por cuenca hidrográfica.">
          <BarChart
            data={view.basinData.map((item) => ({ basin: item.name, volume: item.value }))}
            xKey="basin"
            series={[{ key: "volume", name: "m³ eq" }]}
          />
        </ChartCard>
        ...
        <ChartCard title="Trazabilidad por proceso" description="Procesos que explican la huella total.">
          <BarChart
            data={view.processData.map((item) => ({ process: item.name, volume: item.value }))}
            xKey="process"
            series={[{ key: "volume", name: "m³ eq" }]}
            layout="vertical"
          />
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-5 shadow-soft backdrop-blur">
        <h3 className="text-base font-semibold text-white">Lectura de calidad y estrés</h3>
        <p className="mt-1 text-sm text-slate-400">
          Consolidado base para trazabilidad hídrica, calidad del efluente y riesgos por cuenca.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Estrés alto</p>
            <p className="mt-2 text-2xl font-semibold text-white">{formatCompact(view.kpis.stressHotspots)}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Calidad observada</p>
            <p className="mt-2 text-2xl font-semibold text-white">{formatCompact(view.kpis.qualityIssues)}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Registros trazables</p>
            <p className="mt-2 text-2xl font-semibold text-white">{formatCompact(view.rows.length)}</p>
          </div>
        </div>
      </section>
    </div>
