  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <KpiCard label="Consumo total kWh" value={formatKwh(scope2Kpis.totalKwh)} helper="Demanda eléctrica simulada." />
        <KpiCard label="tCO2e Alcance 2" value={formatTons(scope2Kpis.totalEmissions)} helper="Emisiones eléctricas totales." />
        <KpiCard label="Factor promedio" value={`${scope2Kpis.averageFactor.toFixed(3)} kgCO2e/kWh`} helper="Promedio consolidado." />
        <KpiCard label="% electricidad renovable" value={formatPercent(scope2Kpis.renewableShare)} helper="Cobertura renovable simulada." status="bueno" />
      </section>

      <section className="grid gap-6 xl:grid-cols-2">
        <ChartCard title="Consumo eléctrico mensual" description="Serie de demanda en kWh.">
          <LineChart data={scope2MonthlyEnergyData} xKey="month" series={[{ key: "kwh", name: "kWh" }]} />
        </ChartCard>
        <ChartCard title="Emisiones por instalación" description="Contribución de cada instalación al alcance 2.">
          <BarChart
            data={scope2InstallationData}
            xKey="site"
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-5 shadow-soft backdrop-blur">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">{scope2Explanation}</p>
